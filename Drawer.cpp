...
Drawer::Drawer()
{
    ...
    // cv::namedWindow("Window",cv::WINDOW_AUTOSIZE | cv::CV_GUI_NORMAL);
...
void Drawer::displayPoints(PathStructure ps)
{
    ...
    cv::line(image, cv::Point(ps.gpleft.x + WIN_SIZE_X/2, ps.gpleft.y + WIN_SIZE_Y/2),cv::Point(ps.gpright.x + WIN_SIZE_X/2, ps.gpright.y + WIN_SIZE_Y/2),cv::Scalar(100,255,100));
