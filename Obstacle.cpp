...
void Obstacle::shift_point(Point &p, Obstacle o)
{
    double dx = p.x - o.x;
    double dy = p.y - o.y;
    double rr = this->obstacle_radius/o.obstacle_radius;
    p.x = this->x + rr * dx;
    p.y = this->y + rr * dy;
    p.obstacle_id = this->id;
}
//
...
{
    if(this->id == source_point.obstacle_id || this->id == target_point.obstacle_id)
        return false;
	...
	xtt = (xt*dx + yt*dy)/l;
	ytt = (dx*yt - xt*dy)/l;
	if(ytt <= this->obstacle_radius - ERR && ytt >= -this->obstacle_radius + ERR)
		if(xtt >= ERR && xtt <= l - ERR)
			return true;
	return false;
}
...
bool Obstacle::is_touching(Obstacle target_obstacle)
{
	...
	dl = sqrt(dy*dy + dx*dx);
	return (dl < target_obstacle.obstacle_radius + this->obstacle_radius + ERR);
}

bool Obstacle::encloses(Point p)
{
	double dx,dy,dl;
	dy = p.y - this->y;
	dx = p.x - this->x;
	dl = sqrt(dy*dy + dx*dx);
	return (dl < this->obstacle_radius + ERR);
}
